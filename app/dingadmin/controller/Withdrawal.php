...
class Withdrawal extends Base
{
    ...
    public function index()
    {
        ...
        if ($status != -1) {
            $oModel->where('status', 'in', [1, 4, 5, 2]);
...
//        $url = 'https://payapitest.soon-ex.com/otc/api/issue'; // Gateway
//
...
//    }
    ...
    public function make_xf($order)
    {
        ...
        $dtTmp = date('Ymd');
        $file = fopen("public/pay/soon-exPay_request$dtTmp.txt", "a+");
        $data = [
            'mch_id' => $mch_id,
            'channel_id' => $pay_type,
            'order_sn' => $order['order_sn'],
        ...
        $thirdUserId = uniqid() . '@' . $order['uid'];
        $rsp = (new Security())->widrawcash($amount, $order['order_sn'], $thirdUserId, "");
        $rspArr = json_decode($rsp, true);

        $dtTmp = date('Ymd');
        $file = fopen("public/pay/soon-exPay_rsp$dtTmp.txt", "a+");
        fwrite($file, "返回数据new==AA=\n");
        fwrite($file, $rsp);
        fwrite($file, "\n");
        if (!empty($rspArr['code']) && $rspArr['code'] == 0) {
            return $rspArr;
        }
    ...
    public function make_xf2($order)
    {
        ...
        $dataTmp['remark'] = '提现';
        $dataTmp['version'] = '1.0';
        $bankInfo['bankCode'] = $brank_info['bank_name'];// 银行代码，必填
        $bankInfo['bankName'] = trim($brank_info['bank_name']);// 银行名称，必填
        $bankInfo['cardNumber'] = trim($brank_info['account_no']);// 银行卡的卡号，必填
