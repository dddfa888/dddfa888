...
class User extends Base
{
    ...
    public function index()
    {
        ...
        if (!empty($ids_arr)) {
            $uModel->whereIn('u.admin_id', $ids_arr);
        }
        ...
        $list = $uModel->alias('u')
            ->field("u.*,l.lv_name,sum(r.money) as rujin,sum(w.money) as chujin,uu.account")
            ...
            ->join('admin uu', 'uu.id=u.admin_id', 'LEFT')
            ->order('reg_time desc')
            ->group('u.id')
            ->paginate(['list_rows' => 30, 'query' => request()->param()]);
    ...
    public function add()
    {
        if (request()->isPost()) {
            ...
            $admin_parentinfo = Db::name('admin')->where('id', $user_info['id'])->find();//获取推荐人信息
            if ($admin_parentinfo) {
    ...
    public function edit()
    {
        ...
        $directOrder = 0;
        $lowLevel = Db::name('user')->where('parent_id', $id)->field('id')->select()->toArray();
        if ($lowLevel) {
            $lowLevelIds = array_column($lowLevel, 'id');
            $lowOrderPrice = Db::name('order')->whereIn('user_id', $lowLevelIds)->sum('sumprice');
            if ($lowOrderPrice) {
                $directOrder = $lowOrderPrice;
            }
        }
        return view('edit', [
            'one' => $one,
    ...
    public function achievement()
    {
        ...
        if ($username) {
            $where[] = ['username', '=', input('username/d')];
        }
        $rechargeList = [];
        $withdrawalList = [];
        $rechargeSum = 0;
    ...
    public function explorerExcel()
    {
        // 设置表格的表头数据
        $header = ["A1" => "序号", "B1" => "会员账号", "C1" => "会员等级", "D1" => "手机号码", "E1" => "推荐人(用户)", "F1" => "推荐业务员(代理商)",
            "G1" => "直推人数"
            , "H1" => "注册时间", "I1" => "注册ip", "J1" => "最后登录时间", "K1" => "钱包余额", "L1" => "积分", "M1" => "出金"
            , "N1" => "入金"];
        ...
        $data = [];
        foreach ($list as $row) {
            $data[] = [$row['id'], $row['username'], $row['lv_name'], $row['phone'], $row['parent'], $row['account'],
                $row['parentcount'], date('Y-m-d', $row['reg_time']), $row['regip'], date('Y-m-d', $row['online_time']),
                $row['money'], $row['integral']
                , $row['rujin'], $row['chujin']];
        }
