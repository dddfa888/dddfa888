...
class Login extends BaseController{
    ...
    public function checkLogin () {
    	if (request()->isPost()) {
            ...
            $info = DB::name('user')->where(['username'=>$account,'password'=>md5($password),'is_agent'=>1])->find();
            // dump($info);die;
            if ($info) {
                //添加登录的记录信息
                ...
                session('agent_auth', $admin_indo);
                return json(['status' => 0, 'msg' => '登录成功!', 'url' =>  '/agent/index/index']);
            }else{
